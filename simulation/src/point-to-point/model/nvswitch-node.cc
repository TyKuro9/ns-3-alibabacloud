...
int NVSwitchNode::GetOutDev(Ptr<const Packet> p, CustomHeader &ch){
	...
	if (entry == m_rtTable.end()) {
		if (route_diag) {
			std::fprintf(stderr,
						 "[ns3-route-miss] time=%llu nvswitch=%u protocol=%u sip=%u dip=%u\n",
	...
	if (trace_index < route_trace_limit) {
		std::printf(
				"[ns3-route] time=%llu nvswitch=%u protocol=%u sip=%u dip=%u "
		...
		if (qIndex != 0){ //not highest priority
			if (m_mmu->CheckIngressAdmission(inDev, qIndex, p->GetSize()) && m_mmu->CheckEgressAdmission(idx, qIndex, p->GetSize())){			// Admission control
					...
					if (drop_trace_limit > 0 &&
						...
						std::fprintf(
								stderr,
								"[ns3-admission-drop] count=%llu time=%llu nvswitch=%u "
